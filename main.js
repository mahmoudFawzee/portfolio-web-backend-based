const PROFILE_API_URL = '/api/profile/mamdofa004';
const PROJECTS_API_URL = '/api/projects/mamdofa004';
const SKILLS_API_URL = '/api/skills/mamdofa004';
...
function handleRouting() {
    ...
    } else if (hash === '#home' || hash === '#projects' || hash === '#skills' || hash === '#contact' || hash === '') {
        showHome();
        if (hash && hash !== '#home') {
...
function setupScrollAnimations() {
    const reveals = document.querySelectorAll('section, .glass-card, .project-card, .skill-card');
...
function renderProfile(profile) {
    ...
    profileInfo.professionalBackground.textContent = personalInfo.professionalBackground;
    ...
    contactInfo.forEach(contact => {
        ...
        if (!link.startsWith('tel:') && !link.startsWith('mailto:')) contactItem.target = '_blank';
...
function setupSkillModalEvents() {
    elements.skillModal.close.onclick = () => elements.skillModal.container.style.display = "none";
...
function renderProjectDetails(project) {
    elements.projectDetails.innerHTML = `
        ...
        const iconMap = {
            google_play: 'https://cdn-icons-png.flaticon.com/128/732/732208.png',
            app_store: 'https://cdn-icons-png.flaticon.com/128/888/888841.png',
            youtube: 'https://cdn-icons-png.flaticon.com/128/1384/1384060.png',
            github: 'https://cdn-icons-png.flaticon.com/128/25/25231.png',
            githup: 'https://cdn-icons-png.flaticon.com/128/25/25231.png',
...
function setupModalEvents() {
    elements.modal.close.onclick = () => elements.modal.container.style.display = "none";
    elements.modal.container.onclick = (e) => {
        if (e.target === elements.modal.container) elements.modal.container.style.display = "none";
    };
    elements.modal.prev.onclick = (e) => { e.stopPropagation(); navigateModal(-1); };
    elements.modal.next.onclick = (e) => { e.stopPropagation(); navigateModal(1); };
...
function navigateModal(step) {
    state.currentImageIndex = (state.currentImageIndex + step + state.currentGalleryImages.length) % state.currentGalleryImages.length;
    elements.modal.image.src = state.currentGalleryImages[state.currentImageIndex];
}
